										...
										if (IsValid(self) and IsValid(entity)) then
											...
											entity:CallOnRemove("CreateRation", function()
												if (IsValid(activator)) then
													local rationUniqueID = "low_grade_ration";

													local factionTable = Clockwork.faction:FindByID(activator:GetFaction());
													if (factionTable and factionTable.rationUniqueID) then
														rationUniqueID = factionTable.rationUniqueID;
													end;
													
													-- Spawn the ration
													local itemTable = Clockwork.item:CreateInstance(rationUniqueID);
...
function ENT:Use(activator, caller)
	if (activator:IsPlayer() and activator:GetEyeTraceNoCursor().Entity == self) then
		...
		if (!self.nextUse or curTime >= self.nextUse) then
			local factionTable = Clockwork.faction:FindByID(activator:GetFaction());
			if (factionTable and factionTable.canGetRations and (factionTable.noIDCard or activator:HasItemByID("cid_card"))
				and (!Schema:PlayerIsCombine(activator) or !activator:KeyDown(IN_SPEED))) then
				...
				if (!self:IsLocked() and unixTime >= activator:GetCharacterData("nextration", 0)) then
					if (!self.nextActivateRation or curTime >= self.nextActivateRation) then
						...
						activator:SetCharacterData("nextration", unixTime + Clockwork.config:Get("ration_interval"):Get(720) * 60);
					end;
				elseif (!self:IsLocked() and factionTable.canUseRationToken and activator:HasItemByID("ration_token")) then
